...
try
{
	...
	ResultSet rs2=st.executeQuery("select * from users inner join cart where cart.email='"+email+"' and cart.status='processing'");
	while(rs2.next())
	{
...
<h3>Product Details</h3>
  ...
  ResultSet rs1=st.executeQuery("select * from cart inner join product where cart.product_id=product.id and cart.email='"+email+"' and cart.status='processing'");
  while(rs1.next())
  {
	  sno=sno+1;
